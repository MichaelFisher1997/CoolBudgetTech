...
export default function SimpleSignUpForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    alert('Demo signup form. Please configure Supabase to enable authentication.');
  };

  const handleGoogleSignIn = () => {
    alert('Demo Google sign-in. Please configure Supabase to enable authentication.');
  };
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-lg max-w-md w-full mx-auto">
      ...
      <div className="mb-4 p-3 bg-yellow-100 border border-yellow-400 text-yellow-700 rounded text-sm">
        Demo Mode: Configure Supabase environment variables to enable authentication.
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          type="submit"
          className="w-full bg-indigo-600 text-white py-2 px-4 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:bg-indigo-500 dark:hover:bg-indigo-600"
        >
          Create Account (Demo)
        </button>
      </form>
